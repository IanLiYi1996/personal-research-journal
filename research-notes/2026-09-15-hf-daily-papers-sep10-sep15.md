...
### 🚨🚨🚨 8. 落盘之后发现的一个流程错误：我报的「入库 61 篇」是错的，实际有 11 篇没进库

**我在 CLAUDE.md 那一行里写的是「入库 61 篇（后台逐个跑）」，而落盘后单独跑一次「正文引用的 id 是否都在 bib 里」的对账，答案是 61 条里 11 条不在。**

⭐⭐⭐ **根因不是入库脚本，是我给它包的那层 wrapper**：我让它逐 id 只打印汇总行（`added N, skipped M`），把 `add_paper.py` 自己说明「哪个 id、为什么」的明细行过滤掉了 ⟹ ⚠️⚠️ **而「已在库」与「取文失败」在汇总行里长得一模一样，都是 `added 0, skipped 1`** ⟹ **我读到「50 added + 10 skipped」时读出的是「一切正常」。**

⭐ **真实原因在重跑时一眼可见**（这次我留了明细）：

```
! attempt 1 failed for 2609.11486: HTTP Error 429: Unknown Error
! attempt 2 failed for 2609.11486: The read operation timed out
! attempt 3 failed for 2609.11486: HTTP Error 429: Unknown Error
! attempt 4 failed for 2609.11486: HTTP Error 429: Unknown Error
! fetch failed for 2609.11486 (arXiv + OpenAlex both failed)
```

⟹ **正是 08-12 记过的那个形态：arXiv 429 ＋ 论文太新故 OpenAlex fallback 也没有** —— ⭐ 而它这次之所以没被当场发现，恰恰因为**逐个跑保住了另外 50 篇**（08-12 那次是一批 20 个全军覆没，零产出反而刺眼）⟹ ⭐⭐ **「逐个跑」把一次响的失败换成了一次安静的部分失败，而我此前只记了它的好处。**

⟹ ⭐⭐⭐ **两条判据（已写进 CLAUDE.md）**：①**包一层脚本跑批量入库时，必须逐 id 打印它自己的明细，不要只打印计数** —— 这与「永远不要在 `git add` 上加 `2>/dev/null`」是同一条，⚠️ **而这次更隐蔽：我没有丢弃 stderr，只是让 wrapper 打了一个粒度比被测对象更粗的汇总** ②✅ **新增落盘核验第 ⑤ 项：正文引用的每个 arXiv id 是否都在 `references.bib` 里** —— ⭐ **它比数「入库 N/N」可靠，因为后者用的是我自己维护的那份 id 清单，而这一项直接拿正文去对库。**

⭐ **顺带这次的入库日志第一行还留了一个独立证据，支持我在 `sep15b` 那份里立的「代理 id 探测会给假阴性」**：那次运行开头的探测是 **`arXiv 探测（本会话未取过的 1606.05250）: HTTP 429`**，⭐ **而它随后连续成功取到 50 篇** ⟹ **同一天里第二个实例，且比 `sep15b` 那个更强（那次是 1 篇，这次是 50 篇）。**
