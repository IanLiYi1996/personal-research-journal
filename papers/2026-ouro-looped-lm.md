...
2. **突破循环步数限制**（当前 8→4 的妥协）。可能的方向：循环间 gradient detach、progressive training（逐步增加循环步）、更好的 normalization 方案。
3. **更大规模验证**：1.4B/2.6B 展示了 2-3x 参数效率，7B/13B 规模的 LoopLM 是否能匹配 30B+ 模型？Scaling Law 预测 gap 随规模缩小，值得验证。
4. **与 Mixture-of-Depths 结合**：LoopLM 在循环维度自适应，MoD 在 token 维度自适应，两者正交且可能互补。
5. **LoopLM + OpenClaw-RL 式的在线学习**：循环计算可能天然适合 process reward（每步循环都可以给中间奖励），这与 OpenClaw-RL 的 step-wise PRM 思路契合。
