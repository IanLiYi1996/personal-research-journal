...
无偏性来自：
\[E\!\left[\frac{\binom{n-C}{k}}{\binom nk}\right]=(1-p)^k,\quad C\sim\operatorname{Binomial}(n,p).\]
...
对应的有限样本无偏估计量为：
\[\widehat{\operatorname{pass}^{k}}=\frac{\binom c k}{\binom n k},\qquad 1\le k\le n.\]
当 \(c<k\) 时估计值为 \(0\)；其无偏性为
...
失败比率可写成：
\[\frac{\binom{n-c}{k}}{\binom nk}=\prod_{j=0}^{k-1}\frac{n-c-j}{n-j}=\prod_{j=0}^{k-1}\left(1-\frac{c}{n-j}\right).\]
...
\[\mu=E[S],\qquad\hat\mu=\frac1N\sum_{i=1}^{m}\sum_{j=1}^{n_i}S_{ij},\quad N=\sum_i n_i.\]
...
\[\hat\mu_{\text{micro}}=\frac{\sum_i n_i\bar S_i}{\sum_i n_i}.\]
...
\[\left[\frac{\hat p+\frac{z^2}{2n}-z\sqrt{\frac{\hat p(1-\hat p)}n+\frac{z^2}{4n^2}}}{1+\frac{z^2}n},\quad\frac{\hat p+\frac{z^2}{2n}+z\sqrt{\frac{\hat p(1-\hat p)}n+\frac{z^2}{4n^2}}}{1+\frac{z^2}n}\right].\]
...
\[n_{\text{per arm}}\approx\frac{\left[z_{1-\alpha/2}\sqrt{2\bar p(1-\bar p)}+z_{1-\beta}\sqrt{p_0(1-p_0)+p_1(1-p_1)}\right]^2}{(p_1-p_0)^2},\quad \bar p=\frac{p_0+p_1}{2}.\]
...
\[G=\frac{\sum_dw_ds_d}{\sum_dw_d},\qquad\operatorname{pass}=\mathbb{1}[G\ge\tau]\prod_{d\in C}\mathbb{1}[s_d=1].\]
```python
...
| Judge success | \(TP\) | \(FP\) |
| Judge fail | \(FN\) | \(TN\) |
\[\text{Accuracy}=\frac{TP+TN}{N},\quad\text{Sensitivity}=\text{Success-recall}=\frac{TP}{TP+FN},\]
\[\text{Specificity}=\text{Fail-recall}=\frac{TN}{TN+FP},\quad\text{Balanced accuracy}=\frac{\text{Sensitivity}+\text{Specificity}}2.\]
...
\[p_e=\sum_{c=1}^{K}p_{Ac}p_{Bc},\qquad\kappa=\frac{p_o-p_e}{1-p_e}.\]
...
\[e_{cc'}=\begin{cases}n_cn_{c'}/(n-1),&c\ne c',\\n_c(n_c-1)/(n-1),&c=c'.\end{cases}\]
给定 disagreement function \(\delta(c,c')\)，
\[D_o=\frac1n\sum_{cc'}o_{cc'}\delta(c,c'),\quadD_e=\frac1n\sum_{cc'}e_{cc'}\delta(c,c'),\quad\alpha_K=1-\frac{D_o}{D_e}.\]
...
反解得到：
\[\boxed{p_{\text{true}}=\frac{p_{\text{obs}}-(1-\text{specificity})}{\text{sensitivity}-(1-\text{specificity})}}=\frac{p_{\text{obs}}+Sp-1}{Se+Sp-1}.\]
...
\[\widehat F=\frac{\sum_{i=1}^{N}\sum_{1\le r<s\le R}\mathbb1(V_{ir}\ne V_{is})}{N\binom R2}.\]
...
\[P_{\text{ens-error}}=\sum_{j=(R+1)/2}^{R}\binom Rj e^j(1-e)^{R-j}.\]
该下降依赖独立或弱相关错误。
...
\[C_{\text{total}}=C_{\text{fixed}}+\sum_{i=1}^{m}\sum_{j=1}^{n_i}(C^{agent}_{ij}+C^{env}_{ij}+C^{judge}_{ij}+C^{human}_{ij}).\]
一次 task-trial 的 token cost：
\[C^{agent}_{ij}=\sum_{\ell\in calls}\left(t^{in}_{\ell}r^{in}_{\ell}+t^{out}_{\ell}r^{out}_{\ell}\right)+C^{tool}_{ij}+C^{compute}_{ij},\]
...
独立 strata 下：
\[\widehat{\operatorname{Var}}(\hat p_{\text{strat}})=\sum_hW_h^2(1-f_h)\frac{s_h^2}{n_h},\]
...
\[\text{Trigger-recall}=\frac{TP}{TP+FN},\qquad\text{No-trigger-precision}=\frac{TN}{TN+FN}.\]
另报告：
\[\text{Trigger-precision}=\frac{TP}{TP+FP},\qquad\text{False-trigger-rate}=\frac{FP}{FP+TN}.\]
...
\[J_n(T,O)=\frac{|G_n(T)\cap G_n(O)|}{|G_n(T)\cup G_n(O)|},\quadC_n(T,O)=\frac{|G_n(T)\cap G_n(O)|}{\min(|G_n(T)|,|G_n(O)|)}.\]
...
中位数为 \(\tilde T_h\)。具体规则：
\[\operatorname{Flag}_i=\mathbb1[T_i>3\tilde T_h]\land\mathbb1\!\left[\frac{T_i-\tilde T_h}{1.4826\,MAD_h}>3.5\right].\]
...
无限序列可预先分配：
\[\alpha_t=\frac{\alpha}{t(t+1)},\qquad\sum_{t=1}^{\infty}\alpha_t=\alpha.\]
...
\[UCL_t=\min\!\left(1,\bar p+3\sqrt{\frac{\bar p(1-\bar p)}{n_t}}\right),\quadLCL_t=\max\!\left(0,\bar p-3\sqrt{\frac{\bar p(1-\bar p)}{n_t}}\right).\]
...
\[UCL/LCL_t=\bar p\pmL\sigma_{\hat p}\sqrt{\frac{\lambda}{2-\lambda}\left[1-(1-\lambda)^{2t}\right]}.\]
...
**最小发布规则：**
\[\text{Release}=\mathbb1[L_{\Delta}>-\epsilon]\,\mathbb1[L_{Se}\ge r_S]\,\mathbb1[L_{Sp}\ge r_F]\,\mathbb1[C\le C_{\max}],\]
